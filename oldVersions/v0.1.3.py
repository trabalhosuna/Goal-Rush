...
def jogo():
    ...
    # Carrega as imagens dos jogadores e da bola uma vez fora do loop principal
    ...
    while True:
        ...
        if results.multi_hand_landmarks and len(results.multi_hand_landmarks) >= 2:
            for hand_landmarks in results.multi_hand_landmarks:
                if hand_landmarks == results.multi_hand_landmarks[0]:  # Verifica se é a primeira mão (esquerda).
                    landmark_esquerda = hand_landmarks.landmark[8]  # Acessa a landmark da ponta do indicador.
                    y_esquerda = landmark_esquerda.y
                    print(f"me{y_esquerda}")

                elif hand_landmarks == results.multi_hand_landmarks[1]:  
                    # Acessa as landmarks da segunda mão (direita)
                    landmark_direita = hand_landmarks.landmark[8]
                    y_direita = landmark_direita.y
                    print(f"md{y_direita}")
