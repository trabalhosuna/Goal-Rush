...
"""Versão estavel, primeiro evento adicionado"""
...
class Jogo:
    ...
    class Vida():
        def __init__(self, x, bol):
            self.sentido = bol
            self.x = x
            self.max_vida = 8   # Quantidade de corações
            self.vida = 0
            self.coracao = []
            self.coracao_vazio = []
    ...
    class Evento():
        ...
        def escolher_evento(self):    #escolhe um evento
            self.evento = random.randint(1, 3) #escolhe um evento entre os eventos
            return self.evento

        def evento_vida(self):    #evento vida, cria um coracao no campo e o adiciona ao jogador caso for coletado
    ...
    def abrir_partida(self): # implementa a lógica da partida
        ...
        lucky = True #diz se há caixa no mapa
        evento = 0 #diz qual evento selecionado
        acontecendo_evento = False #diz se o evento está acontecendo
        ...
        while self.rodando_partida: #loop principal da partida
            ...
            if evento == 1:
                ...
                if bola_objeto.rect.colliderect(rect_icon) and verificacao_evento == False:  # Verifica se a bola colidiu com o icone do evento
                    ...
                    print ('colidiu')
                    if ultimo_contato == True: # Verifica se o ultimo contato foi com o jogador direito (bool true sinaliza ultimo contato Direito)
                        vidas_jogador_D.add_coracao()
                        evento = 0
                        verificacao_evento = False
                    if ultimo_contato == False:
                        vidas_jogador_E.add_coracao()
                        evento = 0
                        verificacao_evento = False
