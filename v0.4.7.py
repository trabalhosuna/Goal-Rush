...
"""Versão estavel, implementado novo evento (Portal)"""
...
class Jogo:
    ...
    def abrir_partida(self): # implementa a lógica da partida
        ...
        entrou = False # verifica se a bola entrou no buraco
        ...
        while self.rodando_partida: #loop principal da partida
            ...
            evento
            if self.pontos_E + self.pontos_D >= 1 and verificacao_lucky== False:
            ...
            if lucky:
                ...
                if bola_objeto.rect.colliderect(sorte.caixa):  # Verifica se a bola colidiu com o luckybox
                    ...
                    bola_objeto.velocidade_x -= 2 * componente_normal.x # Subtrai duas vezes o componente da velocidade original da bola para simular a colisão elástica
                    bola_objeto.velocidade_y -= 2 * componente_normal.y
                    evento = 4#sorte.escolher_evento()
                    print(evento)
            ...
            if evento == 4:
                ...
                if bola_objeto.rect.colliderect(rect_icon) and not verificacao_evento:  # verifica
                    print("colidiu")
                    verificacao_evento = True
                    start_evento_4 = True
                    start_tempo = pygame.time.get_ticks()
                    evento = 0

            if start_evento_4:
                if ((pygame.time.get_ticks() - start_tempo) // 1000) == 20:  # verifica se o tempo do evento acabou
                    start_evento_4 = False
                    verificacao_evento = False
                    pass
                else: 
                    rect_portal_1, rect_portal_2 = evento_portal.desenhar_portais()    #desenha os portais
                    if bola_objeto.rect.colliderect(rect_portal_1) and not entrou:
                        entrou = True
                        print(entrou)
                        if bola_objeto.rect.colliderect(rect_portal_1) and entrou:
                            bola_objeto.definir_vetor((rect_portal_2[0], rect_portal_2[1]))
                    elif bola_objeto.rect.colliderect(rect_portal_2) and not entrou:
                        entrou = True
                        print(entrou)
                        if bola_objeto.rect.colliderect(rect_portal_2) and entrou:
                            bola_objeto.definir_vetor((rect_portal_1[0], rect_portal_1[1]))
                    elif  not bola_objeto.rect.colliderect(rect_portal_1) and not bola_objeto.rect.colliderect(rect_portal_2):
                        entrou = False
                        pass
